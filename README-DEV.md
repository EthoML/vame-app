...
`VAME_DATA_ROOT` sets the root folder the in-app file browser may traverse (defaults to your home directory). With the production CLI you'd instead pass `vame-app --data-root /path/to/your/data`, which also remembers it in `~/vame-app/settings.json`.
