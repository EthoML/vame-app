...
class Visualization(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            vame.visualization(**data, save_logs=True)

            if data["parametrization"] == "hmm":
                return dict(
                    result=get_visualization_images(
                        project_path=project_path, parametrization="hmm-15"
                    )
                )
            else:
                return dict(
                    result=get_visualization_images(
                        project_path=project_path, parametrization="kmeans-15"
                    )
                )
